...
{
    gost2012_hash_block((gost2012_hash_ctx *) EVP_MD_CTX_md_data(ctx), (unsigned char*)data,
                        count);
    return 1;
}
...
static int gost_digest_ctrl_256(EVP_MD_CTX *ctx, int type, int arg, void *ptr)
{
    ...
    case EVP_MD_CTRL_MICALG:
        {
            *((char **)ptr) = (char*)OPENSSL_malloc(strlen(micalg_256) + 1);
            if (*((char **)ptr) != NULL) {
...
static int gost_digest_ctrl_512(EVP_MD_CTX *ctx, int type, int arg, void *ptr)
{
    ...
    case EVP_MD_CTRL_MICALG:
        {
            *((char **)ptr) = (char*)OPENSSL_malloc(strlen(micalg_512) + 1);
            if (*((char **)ptr) != NULL) {
